...
if(isset($_POST['studentSearchData'])){
    $regNumber = $_POST['studentSearchData']['regNo']; 
    $level     = $_POST['studentSearchData']['level']; 
    $term      = $_POST['studentSearchData']['term']; 

    if(empty($regNumber)){
        $errors[].='<b>Query error! </b>Make sure the student registration Number is provided!';
        die(displayErrors($errors));//when the registration Number has not been provided kill the script and ech the message out
    }

          if(empty($level) && !empty($term)){
               $query=$db->query("SELECT * FROM filterperformance WHERE registration_number='$regNumber' AND period='$term'");//filterPerofrmace is a view in the databse
          }else if(empty($term) && !empty($level)){
               $query=$db->query("SELECT * FROM filterperformance WHERE registration_number='$regNumber' AND current_grade='$level'");//filterPerofrmace is a view in the databse
          }else if(empty($level) && empty($term)){
               $query=$db->query("SELECT * FROM filterperformance WHERE registration_number='$regNumber' ORDER BY id DESC");//filterPerofrmace is a view in the databse
          }
   ...
   if(mysqli_num_rows($query) == 0){
        $errors[].='<b>Sorry! </b>No data matching your search was found for this student.';
